...
## Redis Configuration (Optional - For Analytics Caching)

To improve analytics performance and reduce external API calls, you can configure Redis caching:

### Vercel KV Setup

Since you're already using Vercel KV, the caching will automatically work with your existing environment variables:
...
- **Reduced Latency**: Cache frequently accessed analytics data
- **Cost Savings**: Fewer external API calls to Plausible
- **Better Performance**: Faster dashboard loading times
- **Smart TTL**: Automatic cache expiration based on data freshness

### Cache Strategy

- **Current Day**: 5-15 minute cache (depends on time of day)
- **Historical Data**: 1 hour cache
- **Yesterday**: 1 hour cache (complete data)
- **Default Periods**: 10 minute cache

### Cache Invalidation

Force refresh analytics data by making a POST request to `/api/invalidate-cache`:
...
fetch('/api/invalidate-cache', {
  ...
  body: JSON.stringify({ type: 'analytics', id: '/p/your-page-path' })
});
...
fetch('/api/invalidate-cache', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({ type: 'player-analytics', id: playerId })
});

// For dashboard analytics (aggregated data)
fetch('/api/invalidate-cache', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({ type: 'dashboard-analytics' })
});
```
...
await invalidateLandingPageCache('/p/your-page-path');
...
When someone submits a contact form, the form owner automatically receives an email notification with the submission details. This feature uses AWS SES for reliable email delivery.
...
SES_SECRET=your_aws_secret_access_key
SES_EMAIL=support@mixflip.io
SES_DISPLAYNAME=MixFlip Support
```

**Note**: These are AWS IAM credentials (Access Key ID and Secret Access Key) with SES permissions. The system now uses the AWS SDK instead of SMTP for better reliability.
...
This will send a test email using the same template that form owners receive.

#### How It Works

1. **Form Submission**: When a contact form is submitted, the data is saved to the `form_submissions` table
2. **Owner Lookup**: The system looks up the form owner's user ID from the `contact_forms` table
3. **Email Retrieval**: The owner's email address is retrieved from Supabase Auth using the admin client
4. **Email Sending**: A formatted HTML email is sent to the form owner with all submission details
5. **Client Identification**: The submitter's name is automatically extracted and displayed in the email
6. **Reply-to Setup**: The reply-to address is automatically set to the form submitter's email
7. **Error Handling**: If email sending fails, the form submission is still saved successfully

#### Security & Protection

The contact form system includes multiple layers of protection:

**Bot Protection:**

- **Turnstile Integration**: Cloudflare Turnstile challenge for human verification
- **Honeypot Fields**: Hidden fields that catch automated form fillers
- **Spam Detection**: Keyword-based content analysis

**Rate Limiting:**

- **IP-based Limits**: Maximum 5 submissions per IP per minute
...
- **Field Count Limits**: Maximum 20 fields per submission
- **Data Structure Validation**: Ensures proper JSON structure
- **Content Sanitization**: Removes potentially malicious content

**Monitoring & Logging:**

- **IP Tracking**: Logs all submission attempts with IP addresses
- **Spam Flagging**: Marks suspicious submissions for review
- **Warning Logs**: Detailed logging of security events
- **User Agent Tracking**: Monitors browser/device information
...
- All form field data in a clean table format
- Professional styling with MixFlip branding
- Fallback text version for email clients that don't support HTML
- **From field** shows "Client Name via MixFlip Contact Forms" (when client name is available)
- **Reply-to address** set to the form submitter's email (when available)
- Clear instructions on how to reply to the client
